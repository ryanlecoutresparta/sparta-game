document.addEventListener('DOMContentLoaded', () => {
  ...
  Game.verticalEnemyMovement = (x, y, size, dx, dy) => {
    console.log("generating...", x, y, size, dx, dy)
    Game.c.beginPath();
...
// parameters
  ...
  let enemy1 = Game.verticalEnemyMovement(x1, y2, sizeSmall, 0, dy1)
  setInterval(enemy1, 10);
  setInterval(Game.verticalEnemyMovement(x2, y2, sizeSmall, 0, dy2), 1000);
  setInterval(Game.verticalEnemyMovement(x3, y3, sizeBig, 0, dy3), 1000);
  setInterval(Game.verticalEnemyMovement(x4, y4, sizeSmall, 0, dy4), 1000);
  setInterval(Game.verticalEnemyMovement(x5, y5, sizeBig, 0, dx5), 1000);
  ...
  levelUp = () => {
    if (Game.player.x <= Game.token.x + 20 && Game.player.y <= Game.token.y + 20 && Game.player.y + 30 >= Game.token.y && Game.player.x >= Game.token.x){
      Game.level1Play = !Game.level1Play;
      Game.score++
      Game.keysPressed = [];
      level2();
    }
  ...
  // setInterval(verticalEnemyMovement, 10);
